...
from kmd_ntx_api.serializers import (
    addressesSerializer,
    balancesSerializer,
    coinLastNtxSerializer,
    coinNtxSeasonSerializer,
    minedSerializer,
    minedCountSeasonSerializer,
    minedCountDailySerializer,
    nnLtcTxSerializer,
    notarisedSerializer,
    notarisedCoinDailySerializer,
    notarisedCountDailySerializer,
    scoringEpochsSerializer,
    coinSocialSerializer,
    notarisedSerializerLite,
    notaryLastNtxSerializer,
    notaryNtxSeasonSerializer,
    serverNtxSeasonSerializer,
    kmdSupplySerializer,
    notarisedTenureSerializer,
)
from kmd_ntx_api.activation import get_activation_commands
from kmd_ntx_api.atomicdex import get_bestorders, get_orderbook
from kmd_ntx_api.query import (
    get_distinct_filters,
    apply_filters_api,
    get_mined_data,
    get_mined_count_season_data,
    get_notary_last_ntx_data,
    get_notary_ntx_season_data,
    get_addresses_data,
    get_coin_ntx_season_data,
    get_coin_social_data,
    get_notarised_tenure_data,
    get_server_ntx_season_data,
)
from kmd_ntx_api.coins import get_dpow_coins_list
...
def get_mined_rows(request):
    source = TableSettings(
        data=models.mined.objects.all(), serializer=minedSerializer, request=request
    )
...
def get_notarised_rows(request):
    ...
    data = cached.get_data(cache_key, expire=900)
    ...
    if data is None:
        ...
        source.required = {"season": season, "coin": coin, "date": f"{date}"}
        ...
        cached.refresh(data=data, force=True, key=cache_key, expire=300)
    return data
...
def get_notary_last_mined_table_api(request):
    season = get_page_season(request)
    season_notary_addresses = get_addresses_data(
        season=season, server="Main", coin="KMD"
    )
    season_notary_addresses = list(
        season_notary_addresses.values_list("address", flat=True)
    )
...
def get_notary_ntx_season_table_data(request, notary=None):
    logger.info("get_notary_ntx_season_table_data")
    seasons_info = get_seasons_info()
    season = get_page_season(request, seasons_info)
    servers = seasons_info[season]["servers"].keys()
            ...
            if notary == item["notary"]:
                for server in servers:
                    ...
                    for coin in server_coins:
                        if coin not in item["server_data"][server]["coins"]:
                            ...
                            coin_ntx_score = 0
                            logger.warning(
                                f'{coin} not in item["server_data"][{server}]["coins"]!'
                            )
                        else:
                            coin_ntx_count = item["server_data"][server]["coins"][coin][
                                "ntx_count"
                            ]
                            coin_ntx_score = item["server_data"][server]["coins"][coin][
                                "ntx_score"
                            ]
                        notary_summary.update(
                            {
                                coin: {
                                    "season": season,
                                    "server": server,
                                    "notary": notary,
                                    "coin": coin,
                                    "coin_ntx_count": coin_ntx_count,
                                    "coin_ntx_score": coin_ntx_score,
                                }
                            }
                        )
                for coin in season_coins:
                    if coin not in item["coin_data"]:
                        ...
                        coin_ntx_count_pct = (
                            item["coin_data"][coin]["pct_of_coin_ntx_count"],
                        )
                        coin_ntx_score_pct = item["coin_data"][coin][
                            "pct_of_coin_ntx_score"
                        ]
                    notary_summary[coin].update(
                        {
                            "coin_ntx_count_pct": coin_ntx_count_pct,
                            "coin_ntx_score_pct": coin_ntx_score_pct,
                        }
                    )
                    ...
                    notary_summary[coin].update(
                        {
                            ...
                            "opret": item["opret"],
                            "since_last_block_time": get_time_since(
                                item["kmd_ntx_blocktime"]
                            )[1],
                        }
                    )
    ...
    api_resp = {
        ...
        "notary_ntx_summary_table": notary_summary
    }
    logger.merge(f"g0t_notary_ntx_season_table_data for {season} {notary}")
    return api_resp
        ...
        for notary in item["notary_data"]:
            if notary in coin_summary[coin]:
                coin_summary[coin][notary].update(
                    {
                        "coin_ntx_count_pct": item["notary_data"][notary][
                            "pct_of_coin_ntx_count"
                        ],
                        "coin_ntx_score_pct": item["notary_data"][notary][
                            "pct_of_coin_ntx_score"
                        ],
                    }
                )
...
def get_mined_24hrs_table(request):
    ...
    data = get_mined_data(None, name, address).filter(block_time__gt=str(days_ago(1)))
    data = data.values()
        ...
        resp.append(
            {
                "season": item["season"],
                "notary": item["notary"],
                "master_server_count": item["notary_data"]["servers"]["KMD"][
                    "ntx_count"
                ],
                "main_server_count": item["notary_data"]["servers"]["Main"][
                    "ntx_count"
                ],
                "third_party_server_count": item["notary_data"]["servers"][
                    "Third_Party"
                ]["ntx_count"],
        ...
        resp.append(
            {
                "season": item["season"],
                "server": item["server"],
                "master_server_count": item["server_data"]["servers"]["KMD"][
                    "ntx_count"
                ],
                "main_server_count": item["server_data"]["servers"]["Main"][
                    "ntx_count"
                ],
                "third_party_server_count": item["server_data"]["servers"][
                    "Third_Party"
                ]["ntx_count"],
...
def get_notary_epoch_scores_table(request, notary=None):
    ...
    notary_ntx_season_data = get_notary_ntx_season_data(season, notary).values()

    rows = []
    totals = {"counts": {}, "scores": {}}

    total_scores = {}
    for item in notary_ntx_season_data:
        notary = item["notary"]
        totals["counts"].update({notary: item["notary_data"]["ntx_count"]})
        totals["scores"].update({notary: item["notary_data"]["ntx_score"]})
        """
...
def get_coin_activation_table(request):
    ...
    json_data = get_activation_commands(request)
                ...
                ui_stats[j].append(
                    {
                        ...
                        "os_pubkey_pct": gui_stats[i]["os"][os]["ui"][ui][
                            "os_pubkey_pct"
                        ],
                        "global_swap_pct": gui_stats[i]["os"][os]["ui"][ui][
                            "global_swap_pct"
                        ],
                        "global_pubkey_pct": gui_stats[i]["os"][os]["ui"][ui][
                            "global_pubkey_pct"
                        ],
                    }
                )
                    ...
                    ui_stats[j].append(
                        {
                            ...
                            "ui": ui,
                            "version": v,
                            "num_swaps": gui_stats[i]["os"][os]["ui"][ui]["versions"][
                                v
                            ]["num_swaps"],
                            "num_pubkeys": len(
                                gui_stats[i]["os"][os]["ui"][ui]["versions"][v][
                                    "pubkeys"
                                ]
                            ),
                            "ui_swap_pct": gui_stats[i]["os"][os]["ui"][ui]["versions"][
                                v
                            ]["ui_swap_pct"],
                            "ui_pubkey_pct": gui_stats[i]["os"][os]["ui"][ui][
                                "versions"
                            ][v]["ui_pubkey_pct"],
                            "num_swaps": gui_stats[i]["os"][os]["ui"][ui]["versions"][
                                v
                            ]["num_swaps"],
                            "os_swap_pct": gui_stats[i]["os"][os]["ui"][ui]["versions"][
                                v
                            ]["os_swap_pct"],
                            "os_pubkey_pct": gui_stats[i]["os"][os]["ui"][ui][
                                "versions"
