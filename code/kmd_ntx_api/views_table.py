...
def rewards_tx_table_view(request):
    context = helper.get_base_context(request)
    context.update({
        "page_title": "Source: Rewards Tx Season",
        "table": "rewards_tx",
        "endpoint": "/api/table/rewards_tx/",
        "filters": ["date"],
        "required": {"date": f"{datetime.date.today()}"},
        "order": [ 1, "desc" ],
        "orderData": { "orderData":[ 3 ],   "targets": [ 4 ] },
        "columns": {
            "txid": "",
            "block_height": "Block Height",
            "address": "Address",
            ...
            "usd_price": "USD Value",
            "block_datetime": "Time UTC"
        }
    })
    return render(request, 'components/tables/generic_source/rewards_tx_table.html', context)
